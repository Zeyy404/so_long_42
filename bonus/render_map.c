...
int	init_textures(t_mlx *mlx)
{
	if (!load_texture(mlx, &mlx->wall_texture, "textures/wall.xpm")
		|| !load_texture(mlx, &mlx->player_texture_idle,
			"textures/player.xpm") || !load_texture(mlx,
			&mlx->player_texture_right, "textures/player1.xpm")
		|| !load_texture(mlx, &mlx->player_texture_left,
			"textures/player2.xpm") || !load_texture(mlx,
			&mlx->player_texture_up, "textures/player3.xpm")
...
static void	assign_texture(t_mlx *mlx, char tile, int x, int y)
{
	...
	anime.offset = roundf(cos(mlx->frame_count * 0.006f + y * 0.5f) * 1.7f);
	if (tile == '1')
	...
	{
		anime.alpha = (sinf(mlx->frame_count * 0.06f) + 1) / 2;
		if (anime.alpha < 0.8f)
			mlx_put_image_to_window(mlx->mlx_ptr, mlx->win_ptr,
